...
<table dir="{{ $direction }}" class="main-table">
    ...
    <tr>
        <td class="main-table-td">
            <img class="mail-img-1" src='{{ dynamicAsset(path: 'public/assets/back-end/img/'.($data['status'] == 'denied' ? 'registration-denied' : 'registration-success').'.png') }}' id="logoViewer" alt="">
            ...
            @elseif($data['status'] == 'approved')
                ...
                <a href="{{route('vendor.auth.login')}}" target="_blank" style="text-decoration: underline">{{url('vendor/auth/login')}}</a>
            @endif
    ...
    <tr>
        <td>
            ...
            <img class="mail-img-2" src="{{ getValidImage(path: "storage/app/public/company/".$companyLogo, type:'backend-logo') }}" id="logoViewer" alt="">
            <span class="privacy">
                    <a href="{{route('privacy-policy')}}" target="_blank" id="privacy-check">{{ translate('Privacy_Policy')}}</a>
                    ...
                    @if ($social_media)
                        @foreach ($social_media as $social)
                            <a href="{{ $social->link }}" target=”_blank”  style="margin: 0 5px;text-decoration:none;">
                                <img src="{{dynamicAsset(path: 'public/assets/back-end/img/'.$social->name.'.png') }}" width="16" alt="">
                            </a>
                        @endforeach
                    @endif
                </span>
